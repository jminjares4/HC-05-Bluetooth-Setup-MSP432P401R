 ...
 *
 * App:
 *      Set the HC-05 Module as a Master using UART. Set the HC-05 in
...
void main(void)
 {
    ...
    sendPutty("Setting Master Module!!\r\n"); // sent message

    P2->OUT |= EN; //set pin as HIGH

    while(s_i < 5){ //iterate over the list
        sendBluetooth(masterCommand[s_i++]); //send command, and increment
    ...
    while(1){
        sendPutty("Master has been configured\r\n");
